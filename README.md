...
1. 场景图静态不变量：无悬空跳转、无"选项全为一次性"的场景、全部场景自起点可达、三个结局均有入口
2. 一次性选项耗尽后仍可离开（逐个场景构造最坏状态）
3. 三种结局定向可达（含"什么都不捡"的沉默兜底路线）
4. 寻名进度门控、证据封顶不可刷、一次性选项用完即消失
5. 设计契约：状态里没有 `san`/`yin`，全程画面文本不含生存数值字样，渗透只以类名与措辞现身
...
11. 五件遗物单局皆可得且不重复
12. 500 局随机播放：无死胡同、无不收敛、随机也能撞到全部三个结局
...
当前 **21/21 精准捕获**。脚本会临时改写 `js/core.js`、`js/chapter-v3.js`、`js/items.js`，运行前应确保工作区没有未保存的重要改动，且不要与其他编辑或测试进程并行；结束时从快照还原并用哈希复核。

有一个变异体最初漏网：把 `addEvidence` 的写侧上限删掉后测试仍全绿，因为读侧 `evidenceScore` 又夹了一次，把缺陷掩盖成等价变异。补了一条只查原始存储字段的断言才真正锁住。
